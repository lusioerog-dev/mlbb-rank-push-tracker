...
export function exportCsv(state: TrackerState) {
  const rows: Array<Array<string | number | null>> = [
    [
      ...
      "star_delta",
      "mythic_checkpoint",
      "rank_checkpoint_kind",
      "rank_checkpoint_tier",
      "rank_checkpoint_division",
      "rank_checkpoint_stars",
      "rank_checkpoint_confirmed_at",
      "rank_checkpoint_reason",
      "rank_tier",
      "source",
      "notes",
    ],
  ...
  for (const m of state.matches)
    rows.push([
      ...
      m.rankCheckpoint?.kind ?? null,
      m.rankCheckpoint?.position.tier ?? null,
      m.rankCheckpoint?.position.division ?? null,
      m.rankCheckpoint?.position.stars ?? null,
      m.rankCheckpoint?.confirmedAt ?? null,
      m.rankCheckpoint?.reason ?? null,
      m.rankTier,
      m.source,
      m.notes,
    ]);
